...
class _TalkToMayaState extends State<TalkToMaya> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _speakingPulseAnimation = Tween<double>(begin: 1.0, end: 1.1).animate(
      CurvedAnimation(parent: _speakingPulseController, curve: Curves.easeInOut),
    );
  }
  ...
  void _onDataMessage() {
    ...
    if (!latest.isFinal) {
      ...
      _shared.currentTranscript = latest.text;
    }

    if (latest.isFinal && latest.text.trim().isNotEmpty) {
      final speaker = latest.speaker == Role.user ? 'user' : 'maya';

      if (_conversation.isEmpty ||
          _conversation.last['text'] != latest.text.trim()) {
        setState(() {
          _conversation.add({
            'type': speaker,
          ...
          _shared.conversation = _conversation;
        });
  ...
  void _onDebugMessage() {
    print('Debug: ${_session!.lastExperimentalMessage}');
  }
  ...
  Future<void> _onStart() async {
    // ✅ BLOCK START if existing session active
